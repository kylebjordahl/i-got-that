...
#   GOOGLE_NATIVE_CLIENT_IDS = <this env's iOS client id AND its Web client id,
#     comma-separated>  (var in wrangler.jsonc per env — see the native app ids
#     table below). Both, because the two platforms mint against different
#     clients — see "Why two audiences" below.
```

**Flutter client:** `lib/state/auth.dart` wires every platform now:
- **Web** navigates to the redirect endpoints (`loginWithGoogle`,
  `connectGoogleCalendar`) as before.
- **Native (iOS and Android)** uses the `google_sign_in` package
  ...
  On **Android** there is no per-flavor client id to wire into the app at all:
  `serverClientId` is the whole configuration, and the Android OAuth clients
  registered in the Cloud Console (one per package name, carrying the signing
  fingerprints) are never referenced by the app or the Worker. No
  `google-services.json` is needed either — that file exists to supply the same
  `serverClientId` the `--dart-define` already provides.

### Why two audiences (`GOOGLE_NATIVE_CLIENT_IDS`)

The platforms disagree about what an id_token's `aud` is, and the list exists
to hold both:

| Platform | `aud` of the id_token | What the OAuth client is for |
| --- | --- | --- |
| iOS | that flavor's **iOS** client id | is itself the audience |
| Android | that env's **Web** client id (the `serverClientId`) | only registers package name + SHA-1 fingerprints; never an audience |

So each env lists its iOS client id **and** its Web client id — the latter
being the same string as that env's `GOOGLE_OAUTH_CLIENT_ID`. Dropping the Web
id breaks Android sign-in with a `401` that reads like a bad token rather than
...
configs. Android uses the same two identifiers as its `applicationId`, so one
device can hold both installs there too:

| Flavor | iOS bundle id (Apple native `aud`) | Android applicationId | Web Services ID |
| --- | --- | --- | --- |
| `staging` | `com.kylebjordahl.igt.staging` | `com.kylebjordahl.igt.staging` | `com.kylebjordahl.igt.web.staging` |
| `prod` | `com.kylebjordahl.igt` | `com.kylebjordahl.igt` | `com.kylebjordahl.igt.web` |
...
Android's half is the hand-written `productFlavors` block in
`apps/mobile/android/app/build.gradle.kts` — deliberately *not* generated from
the `flavorizr:` block, which stays iOS-only (its Android processors rewrite
`build.gradle` wholesale and would clobber the signing config and manifest
placeholders that live in the same file). Keep the two flavor names and the two
app ids in sync by hand.

Each env's `APPLE_CLIENT_IDS` lists that flavor's bundle id **and** its web
Services ID. (Web has no bundle id, and **Sign in with Apple is not offered on
Android** — the package needs an Apple Services ID plus a server callback that
302s to an `intent://` URL, neither of which exists, so the button is hidden
behind `appleSignInAvailable` in `lib/state/auth.dart`.)
...
1. The client passes `<PUBLIC_ORIGIN>/api/auth/google/native-callback` — a
   ...
   (`ASWebAuthenticationSession` on iOS, a Chrome Custom Tab on Android).
2. The user approves; Google redirects to `GET /auth/google/native-callback`
   ...
   query string on to `<GOOGLE_NATIVE_OAUTH_CALLBACK_SCHEME>://google-oauth-callback`.
   The custom-scheme redirect is intercepted on-device — by
   `ASWebAuthenticationSession` on iOS, by `flutter_web_auth_2`'s
   `CallbackActivity` on Android — and handed back to the caller. The browser
   sheet closes automatically: no app switch, no copy/paste. The Worker side is
   identical for both platforms; only the interception differs.
...
- **Google Cloud Console**: add `<PUBLIC_ORIGIN>/api/auth/google/native-callback`
  ...
  registered there).
- **This API**: set `GOOGLE_NATIVE_OAUTH_CALLBACK_SCHEME` per env (a plain var
  in `wrangler.jsonc`) to the custom scheme registered in that flavor's
  `Info.plist`/xcconfig and Android manifest (see below). Unset ⇒
  `/auth/google/native-callback`
  ...
  removing it.
- **Flutter client**, per flavor, in three places that must agree:
  - **iOS** — `GOOGLE_OAUTH_CALLBACK_SCHEME` in
    `ios/Flutter/{staging,prod}*.xcconfig`, registered as a second
    `CFBundleURLTypes` entry in `ios/Runner/Info.plist` (alongside
    `google_sign_in`'s own reversed-client-id scheme).
  - **Android** — the `oauthCallbackScheme` manifest placeholder in each
    `productFlavors` block (`android/app/build.gradle.kts`), consumed by the
    `com.linusu.flutter_web_auth_2.CallbackActivity` intent filter in
    `android/app/src/main/AndroidManifest.xml`.
  - **Dart** — `lib/env.dart`'s `googleOAuthCallbackScheme`, keyed off
    `APP_ENV` rather than a separate dart-define.

  Keep all of those plus `wrangler.jsonc` in sync per flavor. A mismatch has no
