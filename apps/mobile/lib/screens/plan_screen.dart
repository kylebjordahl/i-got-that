...
class _PlanScreenState extends ConsumerState<PlanScreen> {
  ...
  void _onPinchUpdate(ScaleUpdateDetails d) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _gridScroll.jumpTo(
        target.clamp(0.0, _gridScroll.position.maxScrollExtent),
      );
    });
  }
  ...
  late final ScrollController _dayScroll = ScrollController(
    initialScrollOffset: (_dayAnchor - 2) * _dayTileExtent,
  );
  ...
  Widget build(BuildContext context) {
    ...
    final rawTasks =
        ref.watch(allTasksProvider).valueOrNull ?? const <TaskItem>[];
    final events =
    ...
    for (final t in allTasks) {
      ...
      final src = t.calendarEventId == null
          ? null
          : eventsById[t.calendarEventId];
      (src != null && src.allDay ? allDayItems : dayItems).add(
        _PlanItem.task(t),
      );
    }
    final blockEventIds = {
    ...
    for (final cf in conflicts) {
      ...
      if (cf.loser.allDay || cf.winner.allDay || le == null || we == null) {
        continue;
      }
    ...
    // [AppBottomNav]).
    _navClearance =
        MediaQuery.of(context).padding.bottom * 1.4 +
        kBottomNavClearance +
        _midnightMargin;
  ...
  Widget _header() {
    return Row(
      ...
      children: [
        ...
        FiltersButton(
          count: _filterCount,
          onTap: () => _openFilters(caretakersFor()),
        ),
        const SizedBox(width: 8),
        RefreshFeedsButton(
          busy: _refreshingFeeds,
          onTap: _refreshFeeds,
          size: 36,
        ),
      ],
  ...
  Widget _dayScroller(List<TaskItem> allTasks, Map<String, Member> byId) {
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemCount:
            _dayAnchor * 2, // ±27 years — lazily built, effectively infinite
        itemBuilder: (_, i) {
  ...
  Member? _childOf(_PlanItem it, Map<String, Member> byId) {
    if (it.isEvent) {
      final e = it.event!;
      final id = e.isClaimedTask
          ? _taskFor(e)?.familyMemberId
          : e.familyMemberId;
      return byId[id];
    }
  ...
  List<TaskItem> _groupTasksFor(_PlanItem it) {
    ...
    final group = all
        .where((t) => t.calendarEventId == eid && !t.isDismissed)
        .toList();
  ...
  TaskItem _repTask(List<TaskItem> group) => group.firstWhere(
    (t) => t.type == 'attendance',
    orElse: () => group.first,
  );
  ...
  void _openItem(
    ...
    if (group.isNotEmpty) {
      ...
      showEventDetails(
        ...
        ref,
        sourceEvent,
        member: _childOf(it, byId),
        dismissedTasks: _dismissedTasksFor(it),
      );
    }
  }
                  ...
                  children: [
                    for (final it in items)
                      () {
                        final eid = _eventIdOf(it);
                        final source =
                            it.event ?? (eid == null ? null : eventsById[eid]);
                        return _AllDayPill(
                          item: it,
                          sourceEvent: source,
                          accent: personColor(
                            _childOf(it, byId) ?? _fallbackMember,
                          ),
  ...
  Widget _grid(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        double blockLeft(_Placed p) => (laneLeft + p.left * laneWidth).clamp(
          laneLeft,
          laneLeft + laneWidth - blockWidth(p),
        );
        ...
        for (final p in placed) {
          ...
          final edgeTabs = eid == null
              ? const <TaskItem>[]
              : (tabsByEvent[eid] ?? const []);
          final dropoffs = [
            for (final t in edgeTabs)
              if (t.type == 'dropoff') t,
          ];
          final pickups = [
            for (final t in edgeTabs)
          ...
          final sourceEvent =
              p.item.event ?? (eid == null ? null : eventsById[eid]);
        ...
        for (final t in orphanTabs) {
          tabs.add(
            tab(
              t,
              laneLeft + _tabLeftInset,
              laneWidth - 6 - _tabLeftInset,
                ...
                Positioned(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Container(height: 2, color: AppColors.nowLine),
                      ),
                    ],
                  ),
                ),
          ...
          for (final it in items)
            _Ev(
              ...
              // Real end when present; nominal durations for point items.
                  ...
                  ? it.end!.toLocal()
                  : it.start.toLocal().add(
                      Duration(minutes: _isBlock(it) ? 90 : 30),
                    ),
            ),
        ]..sort((a, b) {
  ...
  void _placeCluster(
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      for (var c = col + 1; c < cols.length; c++) {
        final clash = cols[c].any(
          (m) => m.ev.start.isBefore(n.ev.end) && m.ev.end.isAfter(n.ev.start),
        );
        if (clash) break;
        span++;
      }
...
class _DayChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final borderColor = active
        ? AppColors.indigo
        : (isToday ? AppColors.indigo : AppColors.border);
        ...
        SizedBox(
          ...
          child: Text(
            ...
            style: font(
              ...
              11,
              midnight ? 700 : 600,
              color: midnight ? AppColors.textSecondary : AppColors.textMuted,
            ),
          ),
        ),
        Expanded(
...
class _ItemBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final personName = attendees.isNotEmpty
        ? attendees.first.relationName
        : 'child';
                ...
                Positioned(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (showTime) ...[const SizedBox(height: _lineGap), time],
                    ],
                  ),
                ),
              ],
...
class _AllDayPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final e = item.event;
    final summary = e != null
        ? e.displaySummary
        : taskTitle(item.task!, sourceEvent);
    final person = attendees.isNotEmpty
        ? attendees.first.relationName
        : 'child';
