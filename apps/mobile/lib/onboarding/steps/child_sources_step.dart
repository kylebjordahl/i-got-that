...
class ChildSourcesStep extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final feeds =
        ref.watch(memberFeedsProvider(child.id)).valueOrNull ??
        const <FeedItem>[];
    ...
    return OnboardingScaffold(
      ...
      body: [
        Text.rich(
          TextSpan(
            ...
            children: [
              TextSpan(
                text: "$name's",
                style: font(kBodyFont, 14, 600, color: accent),
              ),
              const TextSpan(
                text:
                    ' schedule. Pull from your connected accounts, '
                    'or add a new source.',
              ),
            ],
          ),
        ...
        const InfoHint(
          'A feed can serve several kids — link it here for each '
          'child it applies to.',
        ),
      ],
...
Future<void> _showAddSourceSheet(
...
class _AddSourceSheetState extends ConsumerState<_AddSourceSheet> {
        ...
        for (final c in cals.cast<Map<String, dynamic>>()) {
          out.add(
            _CalCandidate(
              ...
              a.method,
              c['id'] as String,
              (c['name'] as String?) ?? 'Calendar',
              '${a.kindLabel} · ${a.username ?? a.name}',
            ),
          );
        }
      }
  ...
  Future<void> _addAccountCalendar(_CalCandidate c) async {
    await _run(() async {
      ...
      final res = await api.createFeed(
        familyId,
        mode: 'standard',
        kind: c.method,
      ...
      final feedId =
          (res['feed'] as Map<String, dynamic>?)?['id'] as String? ??
          (res['id'] as String);
  ...
  Future<void> _addIcs() async {
    ...
    await _run(() async {
      ...
      final res = await api.createFeed(
        familyId,
        mode: _exception ? 'exception' : 'standard',
        kind: 'ics',
        url: _url.text.trim(),
      );
      final feedId =
          ...
          children: [
            ...
            Padding(
              ...
              child: Align(
                ...
                child: Text(
                  "Add a source for ${widget.child.relationName}",
