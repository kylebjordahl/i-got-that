...
void main() {
  ...
  group('1g runs for every adult, not just the signed-in user', () {
    ...
    test(
      'self still gets a turn when their row is not flagged a caretaker',
      () async {
    ...
    testWidgets(
      "the signed-in user's turn is first and speaks in second person",
      (tester) async {
        await tester.pumpWidget(
          host(
            ParentUnifiedStep(
  ...
  group('summaries receipt what actually happened', () {
        ...
        host(
          ...
          overrides: [
            ...
            familyInfoProvider.overrideWith(
              (ref) async => (name: 'Rivera Family', count: 2),
            ),
    ...
    testWidgets('1h checks off completed steps and flags skipped ones', (
      ...
      expect(
        find.text("They'll pick their own when they join."),
        findsOneWidget,
      );
    ...
    testWidgets('2d flags a join that skipped the calendar pick', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
