...
void main() {
  testWidgets('Plan renders the day scroller + grid without overflow', (
    tester,
  ) async {
  ...
  testWidgets(
    'grid defaults its scroll to 7 AM when an early event expands the window',
    (tester) async {
  ...
  testWidgets(
    'many overlapping calendars pack into narrow columns without overflow',
    (tester) async {
        ...
        ProviderScope(
          overrides: [
            ...
            currentMemberProvider.overrideWith(
              (ref) async => _m('kyle', 'Kyle', caretaker: true),
            ),
            allTasksProvider.overrideWith((ref) async => tasks),
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          currentMemberProvider.overrideWith(
            (ref) async => _m('dad', 'Dad', caretaker: true),
          ),
          allTasksProvider.overrideWith((ref) async => tasks),
          calendarEventsProvider.overrideWith((ref) async => events),
          pendingDecisionsProvider.overrideWith((ref) async => const []),
  ...
  testWidgets('an all-day event rides the pinned row, not a block down the grid', (
    ...
    final events = [
      CalendarEventItem(
        ...
        allDay: true,
        summary: 'Initial parental leave',
      ),
      CalendarEventItem(
        id: 'dentist',
        familyMemberId: 'theo',
  ...
  testWidgets('tapping an event block manages its whole task group', (
    tester,
  ) async {
    final now = DateTime.now();
    DateTime at(int h, int m) => DateTime(now.year, now.month, now.day, h, m);
  ...
  testWidgets('tapping an event block shows the event\'s own details', (
    tester,
  ) async {
    final now = DateTime.now();
    DateTime at(int h, int m) => DateTime(now.year, now.month, now.day, h, m);
  ...
  testWidgets('a free/busy block says why it can never carry a task', (
    ...
    final events = [
      CalendarEventItem(
        ...
        allDay: false,
        summary: 'Busy',
        taskIneligibleReason: 'busy_calendar',
      ),
    ];
    await tester.pumpWidget(
      ProviderScope(
  ...
  testWidgets(
    '"Only my kids" hides a plain calendar event for a child I don\'t cover',
    (tester) async {
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            membersProvider.overrideWith(
              (ref) async => [me, dad, _m('theo', 'Theo', child: true)],
            ),
            currentMemberProvider.overrideWith((ref) async => me),
            allTasksProvider.overrideWith((ref) async => const <TaskItem>[]),
            calendarEventsProvider.overrideWith((ref) async => events),
  ...
  testWidgets(
    'a double-booking flags a Conflict chip that opens the sheet (8a)',
    (tester) async {
      final now = DateTime.now();
      DateTime at(int h, int m) => DateTime(now.year, now.month, now.day, h, m);
  ...
  testWidgets('swiping the grid left/right steps the selected day', (
    tester,
  ) async {
    ...
    await tester.pumpAndSettle();
    expect(
      find.text(longDateComma(today.add(const Duration(days: 1)))),
      findsOneWidget,
    );
    ...
    await tester.pumpAndSettle();
    expect(
      find.text(longDateComma(today.subtract(const Duration(days: 1)))),
      findsOneWidget,
    );
  ...
  group('pinch to zoom the time axis', () {
    ...
    Future<void> pumpDay(
      ...
      List<CalendarEventItem> extra = const [],
    }) async {
      ...
      final events = [
        ...
        CalendarEventItem(
          ...
          familyMemberId: 'theo',
          provenance: 'human',
          start: DateTime(now.year, now.month, now.day, 22),
          end: DateTime(now.year, now.month, now.day, 23),
          allDay: false,
          summary: 'Late thing',
        ),
        CalendarEventItem(
          ...
          familyMemberId: 'theo',
          provenance: 'human',
          start: DateTime(now.year, now.month, now.day, 0, 30),
    ...
    testWidgets('spreading two fingers stretches the hours apart', (
      ...
      expect(
        tester.getRect(find.textContaining('Dentist')).left,
    ...
    testWidgets(
      'an event running past midnight is drawn past the closing one',
      (tester) async {
        final now = DateTime.now();
        final today = DateTime(now.year, now.month, now.day);
        await pumpDay(
