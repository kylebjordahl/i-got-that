...
import { kekStatus } from './lib/secrets.js';
 ...
 * it as a bare `ok`/`missing`/`invalid` (never any key material) so a deploy can
 * be smoke-checked from outside; `ok` goes false when it isn't `ok`, which is
 * what the post-deploy check in .github/workflows/deploy.yml keys off.
 */
app.get('/health', (c) => {
